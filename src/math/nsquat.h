...
struct nsquat
{
    ...
    nsquat<T> &from(const nsvec4<T> &axis_angle_)
    {
        T ang = axis_angle_.w;
        ang /= 2;
    ...
    nsquat<T> &from(const nsvec3<T> &euler_, typename nsvec3<T>::EulerOrder order_)
    {
        T c1, c2, c3, s1, s2, s3;
        c1 = std::cos(euler_.x / 2);
        c2 = std::cos(euler_.y / 2);
        c3 = std::cos(euler_.z / 2);
        s1 = std::sin(euler_.x / 2);
        s2 = std::sin(euler_.y / 2);
        s3 = std::sin(euler_.z / 2);
...
template<class T>
nsquat<T> operator*(const i32 &lhs_, const nsquat<T> &rhs_)
{
    return rhs_ * lhs_;
}
...
using i8quat = nsquat<i8>;
using i16quat = nsquat<i16>;
using iquat = nsquat<i32>;
using i64quat = nsquat<i64>;
using u8quat = nsquat<u8>;
using u16quat = nsquat<u16>;
using uquat = nsquat<u32>;
using u64quat = nsquat<u64>;
