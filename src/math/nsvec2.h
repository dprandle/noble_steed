...
{
    template<class T2>
    explicit nsvec2(const nsvec2<T2> &copy_) : x(static_cast<T>(copy_.x)), y(static_cast<T>(copy_.y))
    {}
    ...
    // string to_string(bool polar_ = false) const
    // {
    //     nsstringstream ss;
    //     if (polar_)
    //         ss << "[" << length() << " " << angle() << "]";
...
template<class T>
nsvec2<T> operator*(const i32 &lhs_, const nsvec2<T> &rhs_)
{
...
// Math typedefs
using cvec2 = nsvec2<char>;
using c16vec2 = nsvec2<char16>;
using c32vec2 = nsvec2<char32>;
using wcvec2 = nsvec2<wchar>;
